const booksJson = [{
    ...
    },
    {
        ...
        "thumbnail": "https://res.cloudinary.com/coding-academy/image/upload/v1603002487/samples/books/9780593213254_gvcuwj.jpg",
        "language": "sp",
        "listPrice": {
            "amount": 19,
            "currencyCode": "USD",
...
function getAllBooks() {
    var books = storageService.loadFromStorage('bookList')
    if (!books) {
        storageService.saveToStorage('bookList', booksJson)
        books = booksJson
    }
...
function addGoogleBook(book) {
    return new Promise(resolve => {
        const bookList = storageService.loadFromStorage('bookList')
        getIdxById(book.id, bookList).then(res => {
            if (res >= 0) return resolve(false)
            bookList.push(book)
            storageService.saveToStorage('bookList', bookList)
            resolve(true)
        })
    })
}

function getIdxById(id, books) {
    return new Promise(resolve => {
        getAllBooks().then(books => {
            resolve(books.findIndex(book => book.id === id))
        })
    })
}
...
function addReview(bookId, newReview) {
    return new Promise(resolve => {
        getAllBooks()
            .then(books => {
                getIdxById(bookId, books).then(bookIdx => {
                    if (!books[bookIdx].review) {
                        books[bookIdx].review = []
                    }
                    books[bookIdx].review.push(newReview)
                    storageService.saveToStorage('bookList', books)
                    resolve(true)
                })
            })
    })
